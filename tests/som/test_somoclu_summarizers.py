...
def one_algo(key, inform_class, summarizer_class, summary_kwargs):
    ...
    spec_data = TableHandle("spec_data", path=testszdata)
    ...
    informer = inform_class.make_stage(
        ...
        aliases={
            "model": "somoclu_inform_test_model",
            "input": "somoclu_inform_test_input",
        },
        **summary_kwargs,
    )
    ...
    full_uncovered_clusters = np.asarray(
        list(np.setdiff1d(np.arange(31 * 31), full_useful_clusters))
    )
...
def test_SomocluSOM_with_columns():
    ...
    summarizerclass = somoclu_som.SOMocluSummarizer
    _, _, _ = one_algo(
        "SOMoclu_wmag", inform_class, summarizerclass, summary_config_dict
    )
...
def test_SomocluSOM_useful_clusters():
    ...
    inform_class = somoclu_som.SOMocluInformer
    summarizerclass = somoclu_som.SOMocluSummarizer
    _, full_useful_clusters, full_uncovered_clusters = one_algo(
        "SOMomoclu1", inform_class, summarizerclass, summary_config_dict
    )
    ...
    summary_config_dict = {
        ...
        "column_usage": "colors",
        "seed": 0,
        "useful_clusters": np.arange(31 * 31),
    }
    inform_class = somoclu_som.SOMocluInformer
    summarizerclass = somoclu_som.SOMocluSummarizer
    ...
    summary_config_dict = {
        ...
        "column_usage": "colors",
        "seed": 0,
        "useful_clusters": full_uncovered_clusters,
    }
    inform_class = somoclu_som.SOMocluInformer
    summarizerclass = somoclu_som.SOMocluSummarizer
...
def test_SomocluSOM_wrong_column():
    ...
    try:
        _ = one_algo(
            "SOMoclu_wrongcolumn", inform_class, summarizerclass, summary_config_dict
        )
    except:
        return
